...
import { fmtClaimDate as fmtDate } from '@/utils/expenseClaims'
const DesktopExpenseClaimsFinalApprovalPage = lazy(() => import('@/pages/desktop/ExpenseClaimsFinalApprovalPage')) // see App.jsx's top comment - split out of the initial bundle

function MobileExpenseClaimFinalApprovalPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      toast.error('Failed to load pending expense claims')
      setItems([])
    }
  }, [])
  ...
  useEffect(() => {
    ...
    if (!appId || !items?.some(i => i.id === appId)) return
    setHighlightId(appId)
    rowRefs.current[appId]?.scrollIntoView({ behavior: 'smooth', block: 'center' })
  ...
  const handleApproveSelected = async () => {
    ...
    setBusy(true)
    try {
      const { data } = await expenseClaimsAPI.finalApprove([...selected])
      const failed = data.results.filter(r => !r.ok)
      const pushFailed = data.results.filter(r => r.ok && r.push_status !== 'pushed')
      if (failed.length === 0 && pushFailed.length === 0) {
        toast.success(`${data.results.length} claim${data.results.length !== 1 ? 's' : ''} approved and posted to ERPNext`)
      } else {
        toast.error(`${data.results.length - failed.length - pushFailed.length} posted cleanly, ${pushFailed.length} had a push issue, ${failed.length} failed`)
      }
      setSelected(new Set())
      load()
    } catch (e) {
  ...
  const handleRejectSelected = async () => {
    ...
    setBusy(true)
    try {
      await expenseClaimsAPI.finalReject([...selected], rejectReason)
      toast.success(`${selected.size} claim${selected.size !== 1 ? 's' : ''} sent back to the Accountant`)
      setSelected(new Set())
      setRejecting(false)
      setRejectReason('')
      load()
      ...
      <div style={{ background: '#fff', borderRadius: 14, border: `1px solid ${c.border}`, overflow: 'hidden' }}>
        {items === null ? (
          ...
          <>
            ...
            {items.map(item => (
              <div key={item.id} ref={el => { rowRefs.current[item.id] = el }} style={{
                display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px',
                borderBottom: `1px solid ${c.bg}`,
                background: highlightId === item.id ? c.orangeBg : 'none',
                ...
                <div style={{ width: 32, height: 32, borderRadius: 9, background: c.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <Icon name="creditCard" size={14} color={c.textSub} />
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: c.text }}>{item.employee_display_name}</div>
                  <div style={{ fontSize: 11, color: c.textMuted, marginTop: 1 }}>
                    {item.project_name || item.project_id} · SAR {item.total_claimed_amount} · {fmtDate(item.accountant_reviewed_at)}
                  </div>
                </div>
              </div>
            ))}
      ...
      {items?.length > 0 && (
        rejecting ? (
          <div style={{ marginTop: 16, background: '#fff', borderRadius: 14, border: `1px solid ${c.border}`, padding: 16 }}>
            ...
            <textarea
              placeholder="Reason for rejecting (required)" value={rejectReason} onChange={e => setRejectReason(e.target.value)} rows={3}
...
export default function ExpenseClaimFinalApprovalPage() {
  const isMobile = useIsMobile()
  return isMobile ? <MobileExpenseClaimFinalApprovalPage /> : <DesktopExpenseClaimsFinalApprovalPage />
}
